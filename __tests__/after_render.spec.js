...
describe("dev logger enabled; prod logger disabled (by default)", () => {
  ["markdown-1", "markdown-2"].forEach((name) => {
    describe(`for ${name}`, () => {
      it("should include dev logger for all pages", async () => {
        const render = await fixture("logger-enabled-dev");
        expect(await render(name)).toMatchSnapshot();
      });
    });
  });
...
describe("prod logger enabled; dev logger disabled", () => {
  ["markdown-1", "markdown-2"].forEach((name) => {
    describe(`for ${name}`, () => {
      it("should include prod logger for all pages", async () => {
        const render = await fixture("logger-enabled-prod");
        expect(await render(name)).toMatchSnapshot();
      });
    });
  });
...
describe("logger config is not specified and env=dev", () => {
  ["markdown-1", "markdown-2"].forEach((name) => {
    describe(`for ${name}`, () => {
      it("should include dev logger for all pages", async () => {
        const render = await fixture("logger-default-dev");
        expect(await render(name)).toMatchSnapshot();
      });
    });
  });
...
describe("logger config is not specified and env=prod", () => {
  ["markdown-1", "markdown-2"].forEach((name) => {
    describe(`for ${name}`, () => {
      it("should not include any loggers", async () => {
        const render = await fixture("logger-default-prod");
        expect(await render(name)).toMatchSnapshot();
      });
    });
  });
